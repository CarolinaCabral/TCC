...
function formataData(campo, evt)
{
    ...
    if((mes > 12) || (dia >31))
    {
        alert("Data incorreta !!! O mês especificado é maior que 12 ou o dia é maior que 31")
        campo.value = "";
    }
    else
    {
...
function formataDataExtrangeira(campo, evt)
{
    var xPosext = PosicaoCursor(campo);
    evt = getEvent(evt);
    var teclaext = getKeyCode(evt);
    if (!teclaValida(teclaext))
        return;
    vrext = campo.value = filtraNumeros(filtraCampo(campo));
    tam = vrext.length;

    if (tam >= 4 && tam < 6)
        campo.value = vrext.substr(0, 4) + '-' + vrext.substr(4);
    if (tam == 6)
        campo.value = vrext.substr(0, 4) + '-' + vrext.substr(4, 2) + '-';
    if (tam > 6)
        campo.value = vrext.substr(0, 4) + '-' + vrext.substr(4, 2) + '-' + vrext.substr(6);

    if(campo.value.length > 9)
    {
    var ano = campo.value.substring(0, 4);
    var mes = campo.value.substring(5, 7);
    var dia = campo.value.substring(8, 10);
    if((mes > 12) || (dia >31))
    {
        alert("Incorrect date!!!the month is higher than 12 or the day is higher than 31.")
        campo.value = "";
    }
    else
    {
    if ((mes == 04 && dia > 30) || (mes == 06 && dia > 30) || (mes == 09 && dia > 30) || (mes == 11 && dia > 30)) 
    {
        alert("Incorrect date!!! This month has 30 days in max.");
        campo.value = "";
    } else { 
        if (ano % 4 != 0 && mes == 2 && dia > 28) {
            alert("Incorrect date!!! This month has 28 days in max.");
            campo.value = "";
        } else { 
            if (ano % 4 == 0 && mes == 2 && dia > 29) {
                alert("Incorrect date!!! This month has 29 days in max.");
                campo.value = "";
            }
            }
        }
...
function Calendario(campo) {        
	...
	todayText: 'Hoje',
	todayStatus: 'Mostra o dia de hoje',
	clearText: 'Limpar',
	clearStatus: 'Limpa a data',
	closeText: 'Fechar',
	closeStatus: 'Fecha o calendário',
	yearStatus: 'Muda o ano',
	monthStatus: 'Muda o mês',
	weekText: 'Sem',
	weekStatus: 'Semana do ano',
	dayStatus: 'DD, M d, yyyy',
	defaultStatus: 'Selecione uma data'
});
}

function CalendarioInt(campo)
{
    $("#"+campo.id).datepick({dateFormat: "yyyy-mm-dd"});
}
...
function abre(iddiv) {
if(iddiv.toString() == "CPF")
{
    document.getElementById(iddiv.toString()).style.display = "block";
    document.getElementById('CNPJ').style.display = "none";
    document.getElementById('NENHUM').style.display = "none";
}
else if(iddiv.toString() == "CNPJ")
{
    document.getElementById(iddiv.toString()).style.display = "block";
...
function ConfirmaDelete(id) {
    var apagar = confirm('Você realmente deseja excluir?');
    if (apagar){
...
function formataMaisculo(campo, evt)
{
    var qualquer = campo.value;
    campo.value = qualquer.toUpperCase();
}
// Formata só números
...
function formataHoraGrande(campo, evt)
{
    //HH:mm
    xPos = PosicaoCursor(campo);
    evt = getEvent(evt);
...
function abrirPopUp() {
    window.open('ConsultarEquipamentos.aspx', '', 'top=0,left=0,menubar=no,toolbar=no,location=no,resizable=no,height=550,width=440,status=no,scrollbars=no,maximize=null,resizable=0,titlebar=no;');
}


function AparecerDivPeloChecked(campo,iddiv)
{
        if(campo.checked)
        {
...
function MarcarTodosCheckbox(idcheckbox) //marca checkbox por botão
{
    $("input[name='"+idcheckbox+"[]']").each(function() {
        $(this).attr("checked", "checked");
    })
}
function Desmarcar(idcheckbox) //desmarca checkbox por botão
{
    $("input[name='"+idcheckbox+"[]']").each(function() {
...
function MarcarCheckBox(campo,idcheckbox) //marca outros checkbox por um checkbox.
{
    ...
    if(campo.checked)
    {
        ...
        for (var i=0;i<document.form1.elements.length;i++) //mais pra frente teria que mandar o nome do form para o document também.
        {
...
function MarcarUmCheck(campo,idcheckbox,posicao)
{
    var chk = document.getElementsByName(idcheckbox); //só funcionou com o ByName e não com o byId
    if(campo.checked)
    {
        chk[posicao].checked = true;
    }
    else
    {
        chk[posicao].checked = false;
    }
}
